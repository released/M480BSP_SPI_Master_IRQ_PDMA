...
#define SPI_GET_UNIT_TRANSFER_FINISH_FLAG(spi)   (((spi)->STATUS & SPI_STATUS_UNITIF_Msk)>>SPI_STATUS_UNITIF_Pos)
...
void SPI_Master_PDMA_PreInit(void)
{
    ...
    PDMA_Open(PDMA, SPI_MASTER_OPENED_CH_TX);
...
{
	// when FIFO empty and BUS not busy
	if ((SPI_GET_TX_FIFO_EMPTY_FLAG(SPI1)) && (!SPI_IS_BUSY(SPI1)))
    {
		set_flag(flag_SPI_Transmit_end,ENABLE);
		SPI_DisableInt(SPI1, SPI_FIFO_TXTH_INT_MASK);
...
void SPI_Master_Init(void)
{
    ...
    SPI_SetFIFO(SPI1, 3, 3);
//	SPI_EnableInt(SPI1, SPI_FIFO_TXTH_INT_MASK);
...
int main()
{
    ...
    while(1)
    {
		...
		if (is_flag_set(flag_SPI_Transmit_timing))	
		{
			...
			if (is_flag_set(flag_SPI_Transmit_end))
			{
